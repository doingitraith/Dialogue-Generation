...
class Productionist(object):
    ...
    def __init__(self, content_bundle_name, content_bundle_directory, probabilistic_mode=False,
        ...
        self.grammar = self._load_grammar(
            grammar_file_location=os.path.join(content_bundle_directory,content_bundle_name)+'.grammar'
        )
    ...
    def _execute_production_rule(self, rule, n_tabs_for_debug):
        ...
        for symbol in rule.body:
            if type(symbol) == str:  # Terminal symbol (no need to expand)
...
class Grammar(object):
    ...
    def __init__(self, grammar_file_location):
        ...
        for rule in self.production_rules:
            for symbol in rule.body:
                if type(symbol) == str and symbol not in self.terminal_symbols:
    ...
    def _init_parse_json_grammar_specification(self, path_to_json_grammar_specification):
        ...
        try:
            ...
            raise Exception(
                "Cannot load grammar -- there is no grammar file located at '{filepath}'. Error: {error}. Working directory:{wd}. Files: {dir}".format(
                    filepath=path_to_json_grammar_specification, error=e, wd = os.getcwd(), dir=os.listdir(os.getcwd())
                )
            )
        ...
        for symbol_id, nonterminal_symbol_specification in nonterminal_symbol_specifications.items():
...
class ProductionRule(object):
    ...
    def __str__(self):
        ...
        return '{head} --> {body}'.format(
            head=self.head,
            body=''.join(symbol if type(symbol) is str else '[[{}]]'.format(symbol.name) for symbol in self.body)
        )
    ...
    def compile_tags(self):
        ...
        for symbol in self.body:
            if type(symbol) != str:  # i.e., if the symbol is nonterminal
